...
interface MessageRepository : CrudRepository<Message, Int> {
	@Query(value = "select * from message where type=?1 and sender=?2 and receiver=?3 and family_id=?4 order by unread, update_at desc", nativeQuery = true)
	fun isExist(type: Int, sender: Int, receiver: Int, familyid: Int): Message?

	@Query(value = "select * from v_message where receiver=?1 order by unread, update_at desc", nativeQuery = true)
